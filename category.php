...
<div class="container mt-5">
  ...
  <h2 class="text-center mb-4">Products in <?php echo $category['name']; ?></h2>
  <div class="row">
    <?php
    if (mysqli_num_rows($result) > 0) {
      while ($row = mysqli_fetch_assoc($result)) {
        ...
        echo '<div class="card">';
        echo '<img src="' . $row['image_main'] . '" class="card-img-top" alt="' . $row['name'] . '">';
        echo '<div class="card-body">';
        echo '<h5 class="card-title">' . $row['name'] . '</h5>';
        echo '<a href="product.php?slug=' . $row['slug'] . '" class="btn btn-primary">View Details</a>';
        echo '</div>';
        echo '</div>';
        echo '</div>';
      }
